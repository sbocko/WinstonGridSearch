 ...
 */
public class SimilarityWorstResultSearch {
...
//        String datasetFilePath = "./other/iris.arff";
        ...
        try {
            ...
            for (Dataset otherDataset : datasetList) {
                ...
                SearchResult worstSearchResultForOtherDataset = databaseConnector.worstSearchResultForDataset(otherDataset);
                SearchResult searchResultFromSS = databaseConnector.similaritySearchForDataset(targetDataset, worstSearchResultForOtherDataset);
                double similaritySearchRmse;
                if (searchResultFromSS == null) {
                    similaritySearchRmse = performNewSearchWithGivenHyperparametersForData(worstSearchResultForOtherDataset, datasetFilePath);
                } else {
                    similaritySearchRmse = searchResultFromSS.getRmse();
                }
                if(worstSearchResultForOtherDataset != null && similaritySearchRmse != -1) {
                    double dissimilarity = databaseConnector.datasetDissimilarityFromSearchResultsRmse(worstSearchResultForOtherDataset.getRmse(), similaritySearchRmse);
